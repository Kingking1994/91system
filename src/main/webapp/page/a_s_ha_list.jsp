...
<!--左侧边栏-->
    ...
    <div class="right_main">
        <div class="right_title title">
    		<span>医院管理员管理</span>
			&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
			<a style="font-size: 20px" href="<%=path%>/page/a_s_ha_detail.jsp"><button>添加</button></a>
    	</div>
      <div class="right_content">
       <div class="order order_list" id="order_list" >
    	  <div class="right_menu">

			  <c:if test="${fn:length(hAdminList) eq 0}">
				  <div class="right_menu_title">
					  <span>没有相关记录</span>
				  </div>
			  </c:if>
			  <c:if test="${fn:length(hAdminList) gt 0}">
    		<div class="right_menu_title">
    			<ul class="clearfix">
    				<li>haid</li>
    				<li>account</li>
				  ...
				  <c:forEach items="${hAdminList}" var="hAdmin">
			  <div class="right_item">
				  <div class="item_detail">
					  <ul class="clearfix">
						  <li>${hAdmin.haid}</li>
						  <li>${hAdmin.account}</li>
						  <li>${hAdmin.password}</li>
						  <li><a href="<%=path%>/sAdmin/hospital_update?hid=${hAdmin.hid}">${hAdmin.hid}</a></li>
						  <li>
							<span>
								<a href="<%=path%>/sAdmin/hAdmin_delete?haid=${hAdmin.haid}">删除</a>
							</span>
						  </li>
					  </ul>
				  </div>
